...
const StaticMathField = dynamic(
    () => import("react-mathquill").then((mq) => mq.StaticMathField),
    {
        ssr: false,
    }
);
...
export default function Home() {
    ...
    const numbersAddedTotal =
        numbers.length == 0 ? null : numbers.reduce((total, n) => total + n, 0);
    const avg =
        numbersAddedTotal == null ? null : numbersAddedTotal / numbers.length;

    const weightsAddedTotal =
        weights.length == 0 ? null : weights.reduce((total, n) => total + n, 0);
    ...
    const variance = varianceTop == null ? null : varianceTop / numbers.length;
        ...
        <>
            <main className="flex flex-col p-6 gap-2">
                ...
                {numbersAddedTotal != null && (
                    <div className="bg-slate-100 p-4 rounded-lg flex flex-col justify-center gap-4">
                        ...
                        <StaticMathField>{`\\overline{x} = \\frac{${numbers.join(
                            " + "
                        )}}{${numbers.length}} = \\frac{${numbersAddedTotal}}{${
                            numbers.length
                        }} = ${avg}`}</StaticMathField>
                    </div>
                )}
                ...
                {variance != null && avg != null && (
                    <div className="bg-slate-100 p-4 rounded-lg flex flex-col justify-center gap-4">
                        <h1 className="text-2xl font-semibold">Wariancja</h1>
                        <StaticMathField>{`W = \\frac{${numbers
                            .map((n) => `(${n} - ${avg})^2`)
                            .join(" + ")}}{${numbers.length}} = \\frac{${numbers
                            .map((n) => `(${n - avg})^2`)
                            .join(" + ")}}{${numbers.length}} = \\frac{${numbers
                            .map((n) => `${(n - avg) ** 2}`)
                            .join(" + ")}}{${
